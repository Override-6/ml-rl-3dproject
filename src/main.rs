...
use crate::component::player_character::{freeze_frozen_players, spawn_player_characters};
...
use crate::simulation::{
    DELTA_TIME, PlayerStepResult, SimulationConfig, SimulationState, TICK_RATE, prepare_sim_state,
    print_simulation_players, reset_simulation, simulation_tick, spawn_players,
};
...
fn create_app(head: HeadMode, player_count: usize, script: Option<Script>) -> App {
    ...
    if head == HeadMode::None {
        ...
        .add_systems(
            Update,
            (mouse_look, mouse_zoom, update_stats_text, debug_controls),
        )
        .add_systems(
            PostUpdate,
            (
    ...
    if let Some(script) = script {
        ...
        add_game_logic_systems!(Update, FixedUpdate, follow_all_script.run_if(not_resetting));
    } else {
        assert_eq!(
            head,
        ...
        add_game_logic_systems!(Update, FixedUpdate, move_player.after(poll_model_directive).run_if(not_resetting));
    }
...
fn run_simulation(mut app: App) {
    println!("sizeof PlayerStep {}", size_of::<PlayerStepResult>());
    app.run();
}
...
fn resetting(cmd: Res<SimulationState>) -> bool {
    cmd.resetting
}

fn not_resetting(cmd: Res<SimulationState>) -> bool {
    !cmd.resetting
}