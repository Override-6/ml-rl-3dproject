...
pub enum ModelDirective {
    ResetSimulation,
    NexStep(Vec<PulseInputSet>, Vec<Vec3>),
}
...
impl ModelCommands {
    ...
    fn poll_model_outputs(&mut self) -> std::io::Result<(Vec<PulseInputSet>, Vec<Vec3>)> {
        ...
        let mut laser_directions = vec![0u8; item_count * size_of::<Vec3>()];
        self.stream.read_exact(&mut laser_directions)?;

        let laser_directions = unsafe {
            std::slice::from_raw_parts(laser_directions.as_ptr() as *const Vec3, item_count)
                .to_vec()
        };
